...
// select 3 things, the form, the amount (input), and the article to display p's
...
form.addEventListener('submit', function(e){
  e.preventDefault();
  //console.log("hello"); // form defaults to send to server, then ?reloads, which
  // is why we include the above preventDefault()
  //const value = amount.value; // generates a string value denoted by black color in dev tools
  const value = parseInt(amount.value); //provides integer denoted by blue color in dev tools
  const random = Math.floor(Math.random() * text.length);

  // need to account for these 3 cases:
  // empty value
  // negative values
  // more than 9, due to array size
